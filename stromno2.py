...
def get_electricity_api_url():
    ...
    return f"https://www.hvakosterstrommen.no/api/v1/prices/{today}_NO2.json"
...
def get_reservoir_data():
    ...
    if response.status_code == 200:
        data = response.json()
        for item in data:
            if item["omrType"] == "EL" and item["omrnr"] == 2:
                return item
    print(f"Failed to retrieve reservoir data. Status code: {response.status_code}")
    return None

webflow_url = "https://api.webflow.com/v2/collections/66a74cd0adfeaa3df4fd230e/items/66a74ceb48aa7b5a14cb711e/live"
